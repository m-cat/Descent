...
extern unsigned int MAX_WIDTH;      /* full allocated space for dungeon */
extern unsigned int MAX_HEIGHT;
extern unsigned int CURRENT_WIDTH, CURRENT_HEIGHT;
extern unsigned int DUNGEON_X, DUNGEON_Y;
extern unsigned int VIEW_WIDTH, VIEW_HEIGHT;
extern unsigned int UI_X, UI_Y;

extern unsigned int TURN_COUNT;

extern unsigned int INPUT_MODE;
extern unsigned int CAMERA_X, CAMERA_Y;
extern unsigned int LOOK_X, LOOK_Y;
extern unsigned int INV_POS;        /* scroll position in inventory */

extern unsigned int TURN_DELAY_ON;
extern unsigned int SCROLL_FACTOR;  /* speed of scrolling - how many tiles to move at once */
...
#define CHK_IN_BOUNDS(y, x)         ((y) < MAX_HEIGHT && (x) < MAX_WIDTH)   /* don't check >= 0 because we use unsigned ints */
#define CHK_IN_DUNGEON(y, x)        ((y) < DUNGEON_Y + CURRENT_HEIGHT && (x) < DUNGEON_X + CURRENT_WIDTH)
...
{
    ITEM            *item;
    unsigned int    n;          /* Number of items of the type */
} ITEM_N;
...
typedef struct
{
    ...
    unsigned int    hp_max, hp_cur;
    unsigned int    mp_max, mp_cur;
