...
/* Due to how frequently walls and floors are, placed, it's prudent to circumvent
 * the inefficient block_create() function
 */
...
/* Resizes a dungeon by amount ABS(resize_x), ABS(resize_y). If resize_x or
 * resize_y are negative, the dungeon will be resized in the negative direction.
...
void dungeon_set_fov() {
	...
	for (i = DUNGEON_Y; i < DUNGEON_Y + CURRENT_HEIGHT; i++) {
		for (j = DUNGEON_X; j < DUNGEON_X + CURRENT_WIDTH; j++) {
			TCOD_map_set_properties(fov_map, j, i, CHK_TRANSPARENT(i, j), CHK_PASSABLE(i, j));
		}
	}
}

DIRECTION random_valid_path(int x, int y, int maze_x, int maze_y, int width, int height) {
	DIRECTION	paths[4];
	int			count = 0;
	/*~~~~~~~~~~~~~~~~~~*/
 ...
 * odd numbers.
 */
void dungeon_gen_maze(int x, int y, int maze_x, int maze_y, int width, int height) {
	enum DIRECTION	d;
	/*~~~~~~~~~~~~~~*/
