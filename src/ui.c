        ...
        case INPUT_ACTION:
            ...
            if (stash != NULL) {
                ...
                if (item_n != NULL) {
                    ...
                    cprint(
                      OFFSET_NOT_Y, OFFSET_NOT_X, TCOD_white, TCOD_black, "%s at your feet. %c%c%c%c%s", sentence, TCOD_COLCTRL_FORE_RGB, 1, 255, 1,
...
void draw_view() {
    ...
    for (i = CAMERA_Y - VIEW_HEIGHT / 2 + 1; i < CAMERA_Y + VIEW_HEIGHT / 2; i++) {
        for (j = CAMERA_X - VIEW_WIDTH / 2; j <= CAMERA_X + VIEW_WIDTH / 2; j++) {
            ...
            if (is_look) {
                TCOD_console_set_char_foreground(NULL, (int) drawj, (int) drawi, TCOD_black);
                TCOD_console_set_char_background(NULL, (int) drawj, (int) drawi, TCOD_amber, TCOD_BKGND_SET);
            }
        }
    }
}
...
void draw_inventory() {
    ...
    cprint_center(
      UI_Y + 3, UI_X + UI_WIDTH / 2 + 1, TCOD_white, TCOD_black, "e%cx%camine | %cu%cse | %cD%crop", TCOD_COLCTRL_1, TCOD_COLCTRL_STOP, TCOD_COLCTRL_1,
      TCOD_COLCTRL_STOP, TCOD_COLCTRL_1, TCOD_COLCTRL_STOP);
    cprint_center(
      UI_Y + 4, UI_X + UI_WIDTH / 2 + 1, TCOD_white, TCOD_black, "%cw%cield | %cW%cear", TCOD_COLCTRL_1, TCOD_COLCTRL_STOP, TCOD_COLCTRL_1, TCOD_COLCTRL_STOP);

    inv_size = (unsigned int) TCOD_list_size(*(player->inventory));
    if (INV_POS >= inv_size) {
        if (inv_size == 0) {
            INV_POS = 0;
        }
    ...
    for (iterator = (ITEM_N **) TCOD_list_begin(*(player->inventory)), i = 0; iterator != (ITEM_N **) TCOD_list_end(*(player->inventory)); iterator++, i++) {
        subject = subject_form((*iterator)->item->art, (*iterator)->n, (*iterator)->item->name);
        if (i == INV_POS) {
            cprint(UI_Y + 6 + i, UI_X + 2, TCOD_white, TCOD_azure, "%c - %s", 'a' + i, subject);
        }
        else {
            cprint(UI_Y + 6 + i, UI_X + 2, (*iterator)->item->col, TCOD_black, "%c - %s", 'a' + i, subject);
        }
...
void draw_ui() {
    ...
    switch (INPUT_MODE) {
        ...
        case INPUT_MESSAGE:
            ...
            for (
              iterator = (char **) TCOD_list_end(message_list) - 1, i = CON_HEIGHT - 2, iterator2 = (unsigned int *) TCOD_list_end(message_turn_list) - 1;
              i > 2 && iterator != (char **) TCOD_list_begin(message_list) - 1; iterator--, iterator2--, i--) {
        ...
        default:
            ...
            if (player->weapon == NULL) {
                ...
                cprint(UI_Y + OFFSET_WEP_Y, UI_X + OFFSET_WEP_X + strlen("Weapon: "), player->weapon->col, TCOD_black, "%s", player->weapon->name);
            }
            ...
            for (
              iterator = (char **) TCOD_list_end(message_list) - 1, i = CON_HEIGHT - 2, iterator2 = (unsigned int *) TCOD_list_end(message_turn_list) - 1;
              i > CON_HEIGHT - OFFSET_MSG + 1 && iterator != (char **) TCOD_list_begin(message_list) - 1; iterator--, iterator2--, i--) {
                if (*iterator2 >= TURN_COUNT) {
                    cprint(i, UI_X + 2, TCOD_white, TCOD_black, "%s", *iterator);
                }
                else {
